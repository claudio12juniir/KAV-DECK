...
export function ClienteAutocomplete({ selecionado, onSelecionar, onLimpar }) {
  if (selecionado) {
    ...
    return (
      <View style={styles.selecionado}>
        ...
        <Badge tone={bloqueado ? "danger" : "success"}>{bloqueado ? "Bloqueado" : "Liberado"}</Badge>
  ...
  return (
    <SearchField
      ...
      buscar={searchClientes}
      onSelecionar={onSelecionar}
      renderItem={(cliente) => (
        <>
          <Text style={styles.opcaoTitulo}>{cliente.participante.razaoSocial}</Text>
          <Text style={styles.opcaoSub}>{cliente.participante.cpfCnpj}</Text>
        </>
      )}
    />
  );
