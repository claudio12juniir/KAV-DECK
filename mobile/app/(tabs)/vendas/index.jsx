...
export default function VendasListScreen() {
  ...
  const carregar = useCallback(async () => {
    setErro("");
    try {
      const { items } = await listPedidosVenda({ status: status || undefined, pageSize: 50 });
      setPedidos(items);
    } catch (err) {
    ...
    <View style={styles.container}>
      ...
      {carregando ? (
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <View style={styles.centro}>
              <Text style={styles.textoMuted}>{erro || "Nenhum pedido de venda encontrado."}</Text>
            </View>
          }
          renderItem={({ item }) => (
            <Pressable style={styles.linha} onPress={() => router.push(`/vendas/${item.id}`)}>
